...
int main()
{
   ...
   SoundBuffer sounbuffer;
    ...
    RectangleShape openingbox1(Vector2f(800,800));
    ...
    win.display();
        ...
        while(win.pollEvent(eev))
        {
            ...
            if(eev.mouseButton.x>310&eev.mouseButton.x<440 &&eev.mouseButton.y>400 &&eev.mouseButton.y<470)
            {
                win.close();
                RenderWindow help(VideoMode(960,720),"Help", Style::Default);

                    Texture helpp;
    if(!helpp.loadFromFile("help.png") )
    {
        printf("error");
    }
    RectangleShape helppp(Vector2f(960,720));
    helppp.setTexture(&helpp);
    helppp.setOrigin(Vector2f(0,0));
    helppp.setPosition(Vector2f(0,0));
               while(help.isOpen())
                {
                    Event ev;

        while(help.pollEvent(ev))
        ...
        help.close();
            ...
            if(eev.mouseButton.x>580&eev.mouseButton.x<720 &&eev.mouseButton.y>600 &&eev.mouseButton.y<700)
            {
                win.close();
            }
            if(eev.mouseButton.x>510&eev.mouseButton.x<615 &&eev.mouseButton.y>50 &&eev.mouseButton.y<110)
            {
                sounddec=1;
                std::cout<<"sound yes \n";
            }

            if(eev.mouseButton.x>640 &&eev.mouseButton.x<750 &&eev.mouseButton.y>50 &&eev.mouseButton.y<110)
            {
                 sounddec=0;
                std::cout<<"sound no \n";
            }

            if(eev.mouseButton.x>70&&eev.mouseButton.x<200 &&eev.mouseButton.y>300 &&eev.mouseButton.y<350)
            {
                 player=2;
                std::cout<<"player two \n";
            }

            if(eev.mouseButton.x>70&&eev.mouseButton.x<200 &&eev.mouseButton.y>380 &&eev.mouseButton.y<435)
            {
                 player=3;
                std::cout<<"player three \n";
            }
            if(eev.mouseButton.x>70&&eev.mouseButton.x<200 &&eev.mouseButton.y>460 &&eev.mouseButton.y<510)
            {
                 player=4;
                std::cout<<"player four \n";
            }
// for board 1
            if(eev.mouseButton.x>515&&eev.mouseButton.x<760 &&eev.mouseButton.y>240 &&eev.mouseButton.y<315)
            {
               win.close();
    ...
    // this is for token
    Texture token1;
    if(!token1.loadFromFile("mangreen.png") )
    {
        printf("error");
    }
    RectangleShape rect1(Vector2f(60,60));
    rect1.setTexture(&token1);
    ...
    Texture token2;
    if(!token2.loadFromFile("manred.png") )
    {
        printf("error");
    }
    RectangleShape rect2(Vector2f(60,60));
    rect2.setTexture(&token2);
    rect2.setOrigin(Vector2f(75,50));
    rect2.setPosition(Vector2f(0,1000));

    Texture token3;
    if(!token3.loadFromFile("manyellow.png") )
    {
        printf("error");
    }
    RectangleShape rect3(Vector2f(60,60));
    rect3.setTexture(&token3);
    rect3.setOrigin(Vector2f(75,50));
    rect3.setPosition(Vector2f(0,1000));

    Texture token4;
    if(!token4.loadFromFile("manblue.png") )
    {
        printf("error");
    }
    RectangleShape rect4(Vector2f(60,60));
    rect4.setTexture(&token4);
    rect4.setOrigin(Vector2f(75,50));
    rect4.setPosition(Vector2f(0,1000));

    //this is for player turn
    RectangleShape playerturn(Vector2f(400,60));
    playerturn.setPosition(Vector2f(1100,100));
    Texture player11;
    player11.loadFromFile("player1.png");

    Texture player22;
    player22.loadFromFile("player2.png");

    Texture player33;
    player33.loadFromFile("player3.png");

    Texture player44;
    player44.loadFromFile("player4.png");
...
playerturn.setTexture(&player11);
...
{
    window.close();
    RenderWindow wint(VideoMode::getDesktopMode(),"welcome page",Style::Default);
    Texture openingbox;

    openingbox.loadFromFile("win1.png");
    Vector2u openingboxsize=openingbox.getSize();
    float openingboxw=static_cast<float>(openingboxsize.x);
    float openingboxh=static_cast<float>(openingboxsize.y);
    RectangleShape openingbox1(Vector2f(1000, 700));
    openingbox1.setTexture(&openingbox);

    while (wint.isOpen())
    {
        Event winn;
         while(wint.pollEvent(winn))

        {

            if (winn.key.code == Keyboard:: Escape)//to close using escape
                {
                wint.close();
                }

        if(winn.type == Event::Closed)
        {
        wint.close();
        }
        if(winn.type==Event::MouseButtonPressed)
        {
            wint.close();
        }
        wint.clear();
        wint.draw(openingbox1);
        wint.display();
    }
...
c=2;
playerturn.setTexture(&player22);
...
if((rect2.getGlobalBounds().left>0 || rect2.getGlobalBounds().left==0) && rect2.getGlobalBounds().left<50 && rect2.getGlobalBounds().top>30 && rect2.getGlobalBounds().top<130)
{
    ...
    openingbox.loadFromFile("win2.png");
    Vector2u openingboxsize=openingbox.getSize();
    float openingboxw=static_cast<float>(openingboxsize.x);
    float openingboxh=static_cast<float>(openingboxsize.y);
    RectangleShape openingbox1(Vector2f(1000, 700));
...
{
    c=3;
    playerturn.setTexture(&player33);
...
else if(rect3.getGlobalBounds().left >0 && rect3.getGlobalBounds().left<150 && rect3.getGlobalBounds().top<70 && rect3.getGlobalBounds().top>0 && (y==6))
{
    ...
    openingbox.loadFromFile("win3.png");
    Vector2u openingboxsize=openingbox.getSize();
    float openingboxw=static_cast<float>(openingboxsize.x);
    float openingboxh=static_cast<float>(openingboxsize.y);
    RectangleShape openingbox1(Vector2f(1000, 700));
...
{
    c=4;
    playerturn.setTexture(&player44);
}
if(y==6 || y==1)
   {
       c=3;
...
if((rect4.getGlobalBounds().left>0 || rect4.getGlobalBounds().left==0) && rect4.getGlobalBounds().left<50 && rect4.getGlobalBounds().top>30 && rect4.getGlobalBounds().top<130)
{
    ...
    openingbox.loadFromFile("win4.png");
    Vector2u openingboxsize=openingbox.getSize();
    float openingboxw=static_cast<float>(openingboxsize.x);
    float openingboxh=static_cast<float>(openingboxsize.y);
    RectangleShape openingbox1(Vector2f(1000, 700));
...
//for board 2
            if(eev.mouseButton.x>515&&eev.mouseButton.x<760 &&eev.mouseButton.y>355 &&eev.mouseButton.y<425)
            {
 win.close();
...
//for board 3
            if(eev.mouseButton.x>515&&eev.mouseButton.x<760 &&eev.mouseButton.y>465 &&eev.mouseButton.y<540)
            {
 win.close();
